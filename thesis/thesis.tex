...
widely implemented standard of Scheme. It takes from it a lot of stnadard library
...
expected in Scheme but not in Common Lisp can be found in \ref{fig:schemevscomlisp}. It
...
a class of potential bugs and is thus more desirable.
...
the standard library, one of the major arguments for zepto as a language.
...
An important argument for adhering to the \gls{r5rs} standard are \glspl{srfi}. They are proposals
of the Scheme programmer community to enhance the standard libraries of the implementations through
the additions of features, much like \glspl{pep} for the Python programming languages and \glspl{eep}
for Erlang. However, the differences that sets the former apart from the latter two is the inclusion
of a reference implementation\footnote{This is not required, but part of the "good tone" of the community
and \gls{srfi}s lacking them are relatively rare.}. Zepto currently supports 5 of these requests by default,
one of which implements a core feature of the language, descriptive custom data types.

It proved to be a major convenience in the past that the implementation of these requests could be
ported with relative ease. By moving away even more from the Scheme standards, this process would
undoubitably be affected in a negative manner. Thus, for both the reference implementation and its
